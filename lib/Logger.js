...
class Logger {
    ...
    command(guild, channel, cmd, author) {
            if (this.logCommands) {
                if (this.logColour)
                    console.log(`${Logger.timestamp}[CMMD]${chalk.magenta.bold(`@${guild ? guild.name : 'Private Message'}: `)}${chalk.green.bold(channel ? `#${channel}: ` : '')}${chalk.yellow.bold(cmd)} was used by ${chalk.cyan.bold(author)}`);
                else
    ...
    info() {
        if (this.logInfo) {
            if(this.logColour)
                console.info(`${Logger.timestamp}[INFO]${chalk.magenta.bold('Nagato: ')}${chalk.cyan.bold(...arguments)}`);
            else
    ...
    warn() {
        if (this.logWarn){
            if (this.logColour)
                console.warn(`${Logger.timestamp}[WARN]${chalk.magenta.bold('Nagato: ')}${chalk.yellow.bold(...arguments)}`);
            else
    ...
    error() {
        if (this.logError){
            if (this.logColour)
                console.error(`${Logger.timestamp}[EROR]${chalk.magenta.bold('Nagato: ')}${chalk.red.bold(...arguments)}`);
            else
    ...
    debug() {
        if (this.logDebug){
            if (this.logColour)
                console.info(`${Logger.timestamp}[DBUG]${chalk.magenta.bold('Nagato: ')}${chalk.black.bold(...arguments)}`);
            else
