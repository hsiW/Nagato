class MessageHandler {
    ...
    help(message, cmdTxt, args) {
        ...
        for (let command in this.bot.commands) {
            if ((!this.bot.commands[command].dm && !message.channel.guild) || ((this.bot.commands[command].adminOnly === "admin" || !this.bot.commands[command].permissionsCheck(message)) && !this.bot.Admins.includes(message.author.id)) || this.bot.commands[command].privateGuildCheck(message))
                continue;
