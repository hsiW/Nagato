class MessageHandler {
	constructor(bot) {
		...
		this.bot = bot;
		this._awaitID = 0;
		this.bot.currentlyAwaiting = new Map();
		this.bot.currentButtons = new Map();
		...
		this.messageHandler = (message) => {
			if ((this.options.userRestricted && !this.options.userRestricted.includes(message.author.id)) ||
				...
				for (var id of this.bot.currentlyAwaiting.keys())
					if (this.bot.currentlyAwaiting.get(id).input(message)) {
						this.bot.currentlyAwaiting.get(id).output(message);
						this.bot.currentlyAwaiting.delete(id);
					}
		}
		...
		this.reactionAddHandler = (message, emoji, userID) => {
			if (userID === this.bot.user.id)
				return;
			if (this.bot.currentButtons.has(message.id) && this.bot.currentButtons.get(message.id).emojis.includes(emoji.name))
				this.bot.currentButtons.get(message.id).add(message, emoji, userID);
		}
		...
		this.reactionRemoveHandler = (message, emoji, userID) => {
			if (userID === this.bot.user.id)
				return;
			if (this.bot.currentButtons.has(message.id) && this.bot.currentButtons.get(message.id).emojis.includes(emoji.name))
				this.bot.currentButtons.get(message.id).remove(message, emoji, userID);
	...
	awaitMessage(input, output, timeout) {
		...
		this._awaitID++;
		this.bot.currentlyAwaiting.set(this._awaitID, {
			input,
			output
		});
		setTimeout(() => {
	...
	reactionButton(emojis, add, remove, messageID, timeout) {
		...
		this.bot.currentButtons.set(messageID, {
			emojis,
			add,
			remove
		});
		setTimeout(() => {
			this.bot.currentButtons.delete(messageID);
