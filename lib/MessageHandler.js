    ...
    constructor(bot, commandsFolder) {
        ...
        this.messageHandler = (message) => {
            if ((this.options.userRestricted && !this.options.userRestricted.includes(message.author.id)) || (message.author.bot && this.options.allowedBots && !this.options.allowedBots.includes(message.author.id))) return;
                ...
                if (message.content.replace(/<@!/, "<@").startsWith(this.bot.user.mention)) {
                    message.content = message.content.replace(/<@!/g, "<@").replace(this.bot.user.mention + ' ', 'mention');
                    if ((message.content.match(new RegExp(this.bot.user.mention, 'g')) || []).length === 0) message.mentions.splice(message.mentions.indexOf(this.bot.user), 1)
                }
                ...
                if (!this.bot.Help.disable && this.bot.Help.helpCommands.includes(cmdTxt)) this.help(message, cmdTxt, args);
...
${helpMessage}
            ...
            var helpFields = [];
            for (let type in sortedHelp) {
                if (!sortedHelp[type].length > 0) continue;
