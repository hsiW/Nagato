class MessageHandler {
    constructor(bot, commandsFolder) {
        ...
        this.messageHandler = (message) => {
            if (message.author.bot || (message.author.bot && options.allowedBots && !options.allowedBots.includes(message.author.id))) return;
                ...
                if (cmdTxt === 'help') this.help(message, args)
    ...
    help(message, args) {
        ...
        this.bot.log.command(message.channel.guild, message.channel ? message.channel.name : undefined, 'help', message.author.username);
    }
