class MessageHandler {
	constructor(bot) {
		...
		this.reactionRemoveHandler = (message, emoji, userID) => {
			...
			if (this.currentButtons.has(message.id) &&
				this.currentButtons.get(message.id).emojis.includes(emoji.name) && this.currentButtons.get(message.id).remove !== null)
	...
	help(message, cmdTxt, args) {
		if (this.bot.commands.size === 0)
			...
			var helpFields = help.map(c => c.type).filter((elem, index, self) => index == self.indexOf(elem)).sort().map(type => {
				return {
					name: type,
