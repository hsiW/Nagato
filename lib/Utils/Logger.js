...
class Logger {
    ...
    command(guild, channel, cmd, author) {
        guild = guild ? guild.name : 'Private Message';
        console.log(chalk.grey.bold(`@${guild}: `) + chalk.green.bold(channel ? `#${channel}: ` : '') + chalk.yellow.bold(cmd) + ' was used by ' + chalk.cyan.bold(author))
    }
